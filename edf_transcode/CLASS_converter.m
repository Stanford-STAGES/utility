...
classdef CLASS_converter < handle
    ...
    methods(Static)
        ...
        %> The user is prompted if outPath does not exist or is not entered.
        % =================================================================
        function unique_comments = mayo2huneo(mayoPath, outPath, comments2remove)
            if ismac                
                addpath('~/git/matlab/file'); % for getFilenamesi
                addpath('~/git/matlab/'); % for showME                
                addpath('~/git/sev/');  % for CLASS_codec.stages2STAGES                
            end
            if(nargin<1)
                disp('Select Directory containing MAYO event (*.txt) and staging (*.csv) files.');
                msg = 'Select MAYO event (*.txt) and staging (*.csv) files.';
                mayoPath = CLASS_converter.getPathname(pwd,msg);
            end
            ...
            if(exist(mayoPath,'dir') && exist(outPath,'dir'))
                % Get all files
                % [a,b,c,d] = getFilenamesi(mayoPath, {'*.txt','*.csv'});
                [~, ~, ~, basenames] = getFilenamesi(mayoPath, {'.csv','txt'});
                
                % This is a simple way to merge basenames which allows me
                % to later identify if I am missing either a .csv or .txt
                % file.
                basenames = unique(basenames);
                skippedFiles = false(size(basenames));
                stageMismatchedFiles = zeros(size(basenames));
                
                numStudies = numel(basenames);
                if numStudies==0
                    fprintf('No studies found for processing.\n');
                else
                    ...
                    for b=1:numStudies
                        try
                            ...
                            event_file = fullfile(mayoPath, [basename, '.txt']);
                            stage_file = fullfile(mayoPath, [basename, '.csv']);
                            if ~exist(event_file,'file')
                                fprintf('%s is missing an event (.txt) file!\n', basename);
                                skippedFiles(b) = true;
                            elseif ~exist(stage_file,'file')
                                fprintf('%s is missing a staging (.csv) file!\n', basename);
                                skippedFiles(b) = true;
                            else
                                fprintf('Processing %s (%d of %d)\n', basename, b, numStudies);
                                
                                staFid = fopen(stage_file, 'r');
                                s = textscan(staFid, '%d,%d', 'headerlines', 1);
                                fclose(staFid);
                                % sort them.
                                [epochs, sortInd] = sort(s{1},'ascend');
                                mayo_scores = s{2}(sortInd);
                                scores = repmat(7, size(mayo_scores));
                                scores(mayo_scores==5) = 0;
                                scores(mayo_scores==1) = 3;
                                scores(mayo_scores==2) = 2;
                                scores(mayo_scores==3) = 1;                                
                                scores(mayo_scores==4) = 5;
                                
                                % % MAYO00002.csv (stages file) example:
                                % position (epoch),Default Staging Set (stage)
                                % 0,5
                                % 1,5
                                ...
                                evtFid = fopen(event_file, 'r');
                                frewind(evtFid);
                                evtChars = fread(evtFid, [1,inf], 'uint16=>char');
                                fclose(evtFid);
                                %     Time	Epoch	Stage	Duration	Title
                                try
                                    e = textscan(evtChars, '%{HH:mm:ss}D %d %s %{mm:ss.S}T %s', 'headerlines', 6,'delimiter',sprintf('\t'));
                                catch me1
                                    e = textscan(evtChars, '%{HH:mm:ss}D %d %s %q %s', 'headerlines', 6,'delimiter',sprintf('\t'));
                                end
                                ...
                                repl_stages = {'W',0;
                                    ...
                                    'R',5;
                                    'U',7};
                                hasStage = ~cellfun(@isempty,evtStage);
                                stageScore = evtStage(hasStage);
                                stageEpoch = evtEpoch(hasStage);
                                for r = 1:size(repl_stages,1)
                                    stageScore(strcmpi(stageScore, repl_stages{r,1})) = {repl_stages{r,2}};
                                end
                                
                                cur_epoch = 0;
                                for r=1:numel(stageScore)
                                    score = stageScore{r};
                                    epoch = stageEpoch(r);                                    
                                    if scores(epoch)~=score && score~=7 && cur_epoch~=epoch
                                        stageMismatchedFiles(b)= stageMismatchedFiles(b)+1;
                                        cur_epoch = epoch;  % just count 1 mismatch per epoch
                                        disp([b, epoch, score, scores(epoch)])
                                    end
                                end
                                
                                if ~isempty(comments2remove)
                                    [~, matchingIndInEvtComment,~] = intersect(evtComment, comments2remove);
                                    evtComment(matchingIndInEvtComment) = {''};
                                    evtComment = strrep(evtComment, ' SHE''s ', ' (PT)''s ');
                                    ...
                                    % in REPLACE, and returns the resulting text as S.
                                    %evtComment = regexpr(evtcomment,'^(s?he\w)|(his|her|him)[ ,.;]?', '(PT)','ignorecase');
                                    test = regexprep(evtComment,'\<(?=s?he|his|her|him)([ ,.;])?', '(PT)$1','ignorecase');
                                    evtComment = regexprep(evtComment,'\<(s?he|his|her|him)\>', '(PT)','ignorecase');
                                    evtComment = regexprep(evtComment,'\<(mom|dad)\>', '(GUARDIAN)','ignorecase');
                                    
%                                     replace_cell = {' She ',' she ',' SHE ',' her ',' Her ',' HER ',' He ', ' he ',' HE ',' HIS ',' his ',' His ',' HIM ',' him ',' Him '}; 
%                                     for r=1:numel(replace_cell)
%                                         evtComment = strrep(evtComment,replace_cell{r},' (PT) ');
%                                     end
%                                     
%                                     replace_cell = {' She ',' she ',' SHE ',' her ',' Her ',' HER ',' He ', ' he ',' HE ',' HIS ',' his ',' His '};
                                    
                                end
                                unique_comments = union(evtComment, unique_comments);
                            end
                        catch me
                            showME(me);
                            skippedFiles(b) = true;
                    ...
                    if any(stageMismatchedFiles)
                        mismatchIndices = stageMismatchedFiles~=0;
                        mismatchedFiles = basenames(mismatchIndices);
                        mismatchedCounts = stageMismatchedFiles(mismatchIndices);
                        numMismatched = numel(mismatchedFiles);
                        fprintf('%d files had mismatched stages:\n', numMismatched);
                        for f=1:numMismatched
                            fprintf('\t%s\t%d\n', mismatchedFiles{f}, mismatchedCounts(f));
                        end
                    end
                    if any(skippedFiles)
                        skippedFiles = basenames(skippedFiles);
                        numSkipped = numel(skippedFiles);
                        fprintf('%d files were skipped:\n', numSkipped);
                        for f=1:numSkipped
                            fprintf('\t%s\n', skippedFiles{f});
                        end
                    else
                ...
                fprintf('\nWarning: mayo2huneo requires a valid input and output path for data conversion.\nNothing done.\n\n');
            end
        end        
        ...
        function sevEvt2db(dbStruct,edf_sta_path,evt_path,samplerate)
            ...
            addpath('/Users/known/git/matlab/file');  % for getFilenames
            addpath('/Users/known/git/sev'); %for loadStages
            ...
            exp_cell = cellfun(@(x) char(x{1}), exp, 'uniformoutput', false);
